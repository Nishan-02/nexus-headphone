...
const defaultItems = [
  {
    ...
    description: 'The foundation of sound.'
  }
];
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active font-bold'}`}>{activeItem.title}</h2>

          <p className={`face-description ${isMoving ? 'inactive' : 'active text-white/60'}`}> {activeItem.description}</p>
